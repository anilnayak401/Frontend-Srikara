...
// ─────────────────────────────────────────────────────────────
// DOCTOR_MEDIA is loaded from shared doctorMedia.js
...
function BlogModal({ blog, onClose }) {
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md" onClick={onClose}>
      ...
      >
        <div className="relative h-64 md:h-72 flex-shrink-0 bg-black">
          {blog.videoUrl ? (
            isYouTube ? (
              <iframe
          ...
          <button onClick={onClose} className="absolute top-4 right-4 w-9 h-9 rounded-full bg-white/95 flex items-center justify-center hover:bg-white shadow transition-all hover:scale-105 z-20">
            <X size={16} />
          </button>
...
function InteractiveBlogsAndVideos({ slug, name, brandAccent }) {
  ...
  const blogs = (doctor && doctor.blogs && doctor.blogs.length > 0) ? doctor.blogs : (rawMediaData.blogs || []);
        ...
        <div className="flex items-center gap-6 shrink-0 ml-auto lg:ml-0">
          {blogs.length > 1 && (
            <div className="flex gap-2">
              <button 
                onClick={scrollLeft} 
                className="w-[44px] h-[44px] rounded-full border border-slate-200 bg-white hover:border-[#8B1A4A] text-slate-600 hover:text-[#8B1A4A] flex items-center justify-center transition-all shadow-sm active:scale-95 hover:shadow-md"
                title="Scroll Left"
              >
              ...
              <div className="relative h-52 w-full overflow-hidden bg-[#F1F5F9] shrink-0 border-b border-[#E2E8F0]">
                ...
                {blog.videoUrl && (
                  <div className="absolute inset-0 flex items-center justify-center bg-black/10 group-hover:bg-black/25 transition-colors z-10">
                    <div className="w-12 h-12 rounded-full bg-white/95 flex items-center justify-center shadow-lg transition-transform duration-300 group-hover:scale-110">
                      <Play size={18} className="text-[#8B1A4A] fill-[#8B1A4A] ml-0.5" />
                    </div>
                  </div>
                )}
                <span 
